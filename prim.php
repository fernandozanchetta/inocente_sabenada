...
$query1 = mysql_query("SELECT * FROM posts WHERE materia = '$user' AND senha = '$pwd'"));
 ...
 for($i =1 ; $i <= count($origem) ; $i++) {
  echo ("$origem[$i] $destino[$i] $custo[$i]<br>");
 }
 ...
 for($x =1 ; $x <= ($nos-1) ; $x++) {
        ...
        if($minimo2 < $minimo1) {
              ...
              $minimo = $minimo1;
              $aux = $aux1;
              echo ("<br> Aresta ($origem[$aux],$destino[$aux]) escolhida de custo $custo[$aux]");
        } else {
              $minimo = $minimo2;
              $aux = $aux2;
              echo ("<br> Aresta ($destino[$aux],$origem[$aux]) escolhida de custo $custo[$aux]");
              $cont = 2;
        }
        if($cont == 1) {
